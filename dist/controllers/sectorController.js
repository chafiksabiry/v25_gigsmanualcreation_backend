...
class SectorController {
    ...
    static async getAllSectors(req, res) {
        try {
            ...
            const sectors = await sectorModel_1.Sector.find(query)
                .sort({ name: 1 });
            res.status(200).json({
                success: true,
                data: sectors
    ...
    static async getSectorById(req, res) {
        try {
            ...
            const sector = await sectorModel_1.Sector.findById(sectorId);
            if (!sector) {
    ...
    static async createSector(req, res) {
        try {
            ...
            const existingSector = await sectorModel_1.Sector.findOne({ name: sectorData.name });
            if (existingSector) {
            ...
            const sector = new sectorModel_1.Sector(sectorData);
            await sector.save();
            res.status(201).json({
                success: true,
                data: sector,
    ...
    static async updateSector(req, res) {
        try {
            ...
            const sector = await sectorModel_1.Sector.findByIdAndUpdate(sectorId, updateData, { new: true, runValidators: true });
            if (!sector) {
                return res.status(404).json({
                    success: false,
                    error: 'Sector not found'
    ...
    static async deleteSector(req, res) {
        try {
            const sectorId = req.params.id;
            const sector = await sectorModel_1.Sector.findByIdAndDelete(sectorId);
            if (!sector) {
                return res.status(404).json({
                    success: false,
                    error: 'Sector not found'
