...
const bulkDataProcessor_1 = require("../utils/bulkDataProcessor");
...
class BulkController {
     ...
     */
    static async processBulkGigs(req, res) {
        try {
            const { data } = req.body;
            ...
            const options = {
                ...
                validateFunction: (item) => {
                    ...
                    if (item.activities) {
                        if (!Array.isArray(item.activities)) {
                            ...
                            const invalidActivities = item.activities.filter((id) => !mongoose_1.default.Types.ObjectId.isValid(id));
                    ...
                    if (item.industries) {
                        if (!Array.isArray(item.industries)) {
                            ...
                            const invalidIndustries = item.industries.filter((id) => !mongoose_1.default.Types.ObjectId.isValid(id));
                ...
                processFunction: async (chunk) => {
                    // Traitement par chunk avec gestion d'erreurs
                    try {
                        const result = await gigModel_1.Gig.insertMany(chunk, {
            ...
            const result = await bulkDataProcessor_1.BulkDataProcessor.processBulkData(data, options);
            // Calculer les statistiques finales
            const totalInDatabase = await gigModel_1.Gig.countDocuments();
            const response = {
                success: true,
     ...
     */
    static async processBulkCountries(req, res) {
        try {
            ...
            const options = {
                ...
                processFunction: async (chunk) => {
                    ...
                    try {
                        const result = await countryModel_1.Country.insertMany(normalizedChunk, {
                            ordered: false,
                            rawResult: true
                        });
            ...
            const result = await bulkDataProcessor_1.BulkDataProcessor.processBulkData(countries, options);
            const totalInDatabase = await countryModel_1.Country.countDocuments();
            const response = {
                success: true,
                validation: result.validation,
                processing: result.processing,
     ...
     */
    static async validateDataset(req, res) {
        try {
            ...
            switch (type) {
                case 'gigs':
                    validateFunction = bulkDataProcessor_1.BulkDataProcessor.validateGigData;
                    break;
                case 'countries':
                    validateFunction = bulkDataProcessor_1.BulkDataProcessor.validateCountryData;
                    break;
                default:
                    return res.status(400).json({
                        success: false,
     ...
     */
    static getValidationRecommendations(result, type) {
