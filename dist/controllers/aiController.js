...
class AIController {
     ...
     */
    static async generateSkills(req, res) {
        try {
            ...
            const skills = await aiService_1.AIService.generateSkills(title, description || '', skillsData, languagesData);
     ...
     */
    static async generateTimezones(req, res) {
        try {
            ...
            const timezoneResponse = await aiService_1.AIService.generateTimezones(request);
     ...
     */
    static async generateDestinations(req, res) {
        try {
            ...
            const destinations = await aiService_1.AIService.generateDestinations(title, description || '', category || '');
     ...
     */
    static async testGigSuggestions(req, res) {
        try {
            const { description } = req.body;
            if (!description) {
                return res.status(400).json({
            ...
            const testResponse = {
                ...
                category: category,
                destination_zone: timezonesData.find((tz) => tz.zoneName === "Europe/Paris")?._id ||
                    timezonesData.find((tz) => tz.zoneName === "UTC")?._id || "UTC",
                activities: activitiesData.slice(0, 3).map((activity) => activity._id),
                industries: industriesData.slice(0, 2).map((industry) => industry._id),
                seniority: {
                    level: "Mid-Level",
                    yearsExperience: 2
                },
                skills: {
                    languages: languagesData.slice(0, 2).map((lang) => ({
                        language: lang._id,
                        proficiency: "B2",
                        iso639_1: lang.code || 'en'
                    })),
                    soft: skillsData.soft.slice(0, 3).map((skill) => ({
                        skill: skill._id,
                        level: 3,
                        details: skill.description
                    })),
                    professional: skillsData.professional.slice(0, 3).map((skill) => ({
                        skill: skill._id,
                        level: 3,
                        details: skill.description
                    })),
                    technical: skillsData.technical.slice(0, 3).map((skill) => ({
                        skill: skill._id,
                        level: 3,
                        details: skill.description
                    }))
                ...
                availability: {
                    ...
                    time_zone: timezonesData.find((tz) => tz.zoneName === "Europe/Paris")?._id ||
                        timezonesData.find((tz) => tz.zoneName === "UTC")?._id || "UTC",
                ...
                team: {
                    ...
                    territories: [countriesData.find((country) => country.name?.common === "France")?._id || "France"]
                },
     ...
     */
    static async testApiConnections(req, res) {
        try {
     ...
     */
    static async testPopulateData(req, res) {
        try {
            ...
            // Test du populate
            const populatedData = await populateService_1.PopulateService.populateGigData(sampleGigWithIds, {
                activities: true,
                industries: true,
                languages: true,
                skills: true
     ...
     */
    static async testActivityMapping(req, res) {
        try {
