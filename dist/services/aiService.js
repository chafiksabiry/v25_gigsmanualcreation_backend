...
const getAnthropicClient = () => {
    ...
    if (!key)
        return null;
    anthropic = new sdk_1.default({ apiKey: key });
    return anthropic;
};
...
function openaiSupportsJsonMode(model) {
...
function shouldFallbackToClaude(err) {
    ...
    if (code && ['rate_limit_exceeded', 'insufficient_quota', 'server_error', 'service_unavailable', 'overloaded'].includes(code))
        return true;
...
async function callLLMWithFallback(opts) {
    const { systemPrompt, userPrompt, openaiModel, temperature = 0.7, maxTokens = 2000, forceJson = false, } = opts;
    const resolvedModel = forceJson && !openaiSupportsJsonMode(openaiModel) ? DEFAULT_OPENAI_MODEL : openaiModel;
    if (forceJson && resolvedModel !== openaiModel) {
        console.log(`ℹ️ Modèle OpenAI "${openaiModel}" sans mode JSON → utilisation de "${resolvedModel}"`);
    }
    try {
        ...
        console.error(`❌ Erreur OpenAI (fallback Claude = ${fallback}):`, openaiError?.message || openaiError);
        if (!fallback)
            throw openaiError;
        ...
        const claudeSystem = forceJson
            ...
            : systemPrompt;
        let lastClaudeError = openaiError;
        ...
        console.error('❌ Tous les modèles Claude ont échoué:', lastClaudeError?.message || lastClaudeError);
...
class AIService {
    ...
    static extractMongoId(value) {
        ...
        if (typeof value === 'object') {
            const obj = value;
            if (obj.$oid != null)
                return this.extractMongoId(obj.$oid);
            if (obj._id != null)
                return this.extractMongoId(obj._id);
        }
        return null;
    }
    static getCountryCommonName(country) {
    ...
    static getEntityLabel(entity) {
        ...
        if (typeof entity.name === 'string')
            return entity.name;
        if (entity.name?.common)
            return entity.name.common;
        return String(entity.code || '');
    }
    /** Pick the timezone whose document matches the country (by countryCode or by IANA zone). */
    static findTimezoneForCountry(countryDoc, timezonesList) {
        if (!countryDoc || !timezonesList?.length)
            return null;
        ...
        if (ianaZone) {
            ...
            if (exact)
                return { id: String(exact._id), zoneName: exact.zoneName || exact.name || ianaZone };
        }
        ...
        if (ianaZone) {
            ...
            if (byZone)
                return { id: String(byZone._id), zoneName: ianaZone };
        }
        return null;
    }
    static getCountryById(countriesData, countryId) {
        if (!countriesData?.length || !countryId)
            return null;
    ...
    static enrichGigResponseMeta(parsedResponse, countriesData, timezonesData, currenciesData) {
        ...
        const tzId = this.extractMongoId(parsedResponse.availability?.time_zone ?? parsedResponse.schedule?.time_zone);
    ...
    static normalizeDestinationZone(rawValue, countriesData, description) {
     ...
     */
    static findBestCategory(suggestedCategory) {
        if (!suggestedCategory)
            return 'Customer Service'; // Default
        // Recherche exacte
        const exactMatch = PREDEFINED_CATEGORIES.find(cat => cat.toLowerCase() === suggestedCategory.toLowerCase());
        if (exactMatch)
            return exactMatch;
        ...
        const partialMatch = PREDEFINED_CATEGORIES.find(cat => cat.toLowerCase().includes(lowerSuggested) ||
     ...
     */
    static analyzeDescriptionForCountry(description, countriesData) {
        if (!countriesData || countriesData.length === 0) {
        ...
        for (const country of countriesData) {
            const namesToCheck = [
                ...
                ...Object.values(country.name?.nativeName || {}).flatMap((native) => [
     ...
     */
    static findCountryId(countryName, countriesData, fullContext) {
        if (!countriesData || countriesData.length === 0) {
        ...
        let country = countriesData.find((c) => c.name?.common?.toLowerCase() === normalizedName);
        if (country) {
            console.log(`✅ Trouvé par nom commun: ${country.name.common} (${country.cca2}) → ${country._id}`);
            return country._id;
        }
        // 2. Recherche par nom officiel exact
        country = countriesData.find((c) => c.name?.official?.toLowerCase() === normalizedName);
        if (country) {
        ...
        country = countriesData.find((c) => c.name?.common?.toLowerCase().includes(normalizedName) ||
        ...
        country = countriesData.find((c) => c.name?.official?.toLowerCase().includes(normalizedName) ||
     ...
     */
    static findCountryCode(countryName, countriesData) {
        if (!countriesData || countriesData.length === 0) {
        ...
        if (countryName.length === 2) {
            const country = countriesData.find((c) => c.cca2?.toLowerCase() === normalizedName);
            if (country)
                return country.cca2;
        }
        ...
        if (!country) {
            ...
            if (mappedCode) {
                country = countriesData.find((c) => c.cca2 === mappedCode);
            }
        }
    ...
    static async generateGigSuggestions(description, activitiesData, industriesData, languagesData, skillsData, timezonesData, countriesData, currenciesData) {
        ...
        const currencyOptions = sortedCurrencies
            .slice(0, 25)
            .map((currency) => `${currency.code} (${currency.name}, ${currency.symbol}): ${currency._id}`)
            .join('\n');
        if (sortedCurrencies.length === 0) {
            console.warn('⚠️ Aucune devise disponible pour OpenAI - currency sera défaillant');
        }
        else {
            console.log(`💰 ${sortedCurrencies.length} devises envoyées au prompt (top: ${sortedCurrencies.slice(0, 3).map((c) => c.code).join(', ')})`);
        }
...
JSON format:
{
  ...
  "team": {
        ...
        return retryWithBackoff(async () => {
            ...
            try {
                ...
                if (parsedResponse.skills?.languages) {
                    parsedResponse.skills.languages = parsedResponse.skills.languages.map((lang) => ({
                ...
                if (parsedResponse.skills) {
                    if (parsedResponse.skills.soft) {
                        parsedResponse.skills.soft = parsedResponse.skills.soft.map((s) => ({
                    ...
                    if (parsedResponse.skills.professional) {
                        parsedResponse.skills.professional = parsedResponse.skills.professional.map((s) => ({
                    ...
                    if (parsedResponse.skills.technical) {
                        parsedResponse.skills.technical = parsedResponse.skills.technical.map((s) => ({
                ...
                const enforcedTimezoneName = enforcedTimezone?.zoneName ||
                ...
                if (parsedResponse.schedule?.schedules && timezonesData) {
                    parsedResponse.schedule.schedules = parsedResponse.schedule.schedules.map((schedule) => ({
                        ...schedule,
                        timezone: enforcedTimezoneId ||
                ...
                if (parsedResponse.availability && timezonesData) {
                    ...
                    const timezoneId = enforcedTimezoneId ||
                ...
                if (parsedResponse.team?.territories && countriesData) {
                    ...
                    parsedResponse.team.territories = parsedResponse.team.territories.map((territory) => {
                ...
                if (parsedResponse.schedule?.timeZones && timezonesData) {
                    parsedResponse.schedule.timeZones = parsedResponse.schedule.timeZones.map((tz) => this.findTimezoneId(tz, timezonesData, timezoneContext));
                }
     ...
     */
    static async generateSkills(title, description, skillsData, languagesData) {
        if (!this.isValidApiKey()) {
            throw new Error('OpenAI API key not configured properly');
        }
...
Return JSON in this exact format:
{
  ...
  "technical": [{"skill": "skillName", "level": 4, "details": "Why this skill is relevant"}]
        ...
        return retryWithBackoff(async () => {
            ...
            try {
                ...
                if (parsedResponse.languages) {
                    parsedResponse.languages = parsedResponse.languages.map((lang) => ({
                        language: this.findLanguageId(lang.language, languagesData),
                        proficiency: lang.proficiency,
                        iso639_1: lang.iso639_1
                    }));
                ...
                if (parsedResponse.soft) {
                    parsedResponse.soft = parsedResponse.soft.map((s) => ({
                        skill: this.findSkillId(s.skill, skillsData.soft),
                        level: s.level,
                        details: s.details
                    }));
                ...
                if (parsedResponse.professional) {
                    parsedResponse.professional = parsedResponse.professional.map((s) => ({
                        skill: this.findSkillId(s.skill, skillsData.professional),
                        level: s.level,
                        details: s.details
                    }));
                ...
                if (parsedResponse.technical) {
                    parsedResponse.technical = parsedResponse.technical.map((s) => ({
                        skill: this.findSkillId(s.skill, skillsData.technical),
                        level: s.level,
                        details: s.details
                    }));
     ...
     */
    static async generateTimezones(request) {
        if (!this.isValidApiKey()) {
            throw new Error('OpenAI API key not configured properly');
        }
...
Format your response as a JSON object with the following structure:
{
  ...
  "flexibilityRecommendations": ["Recommendation 1", "Recommendation 2", ...]
     ...
     */
    static async generateDestinations(title, description, category) {
        if (!this.isValidApiKey()) {
            throw new Error('OpenAI API key not configured properly');
        }
...
Example response format: ["US", "CA", "UK", "DE"]`;
     ...
     */
    static findSkillId(skillName, skillsList) {
        // Exact match first
        ...
        skill = skillsList.find(s => s.name.toLowerCase().includes(skillName.toLowerCase()) ||
     ...
     */
    static findCurrencyId(currencyCode, currenciesList) {
     ...
     */
    static findLanguageId(languageName, languagesList) {
     ...
     */
    static findTerritoryId(territoryName, countriesList) {
        ...
        let territory = countriesList.find(country => country.name?.common?.toLowerCase() === territoryName.toLowerCase());
        if (territory)
            return territory._id;
        // Recherche par nom officiel
        territory = countriesList.find(country => country.name?.official?.toLowerCase() === territoryName.toLowerCase());
        if (territory)
            return territory._id;
        // Recherche partielle par nom de pays (common)
        territory = countriesList.find(country => country.name?.common?.toLowerCase().includes(territoryName.toLowerCase()));
        if (territory)
            return territory._id;
        // Recherche partielle par nom officiel
        territory = countriesList.find(country => country.name?.official?.toLowerCase().includes(territoryName.toLowerCase()));
        if (territory)
            return territory._id;
        ...
        if (mappedCountry) {
            territory = countriesList.find(country => country.name?.common?.toLowerCase() === mappedCountry.toLowerCase() ||
     ...
     */
    static findTimezoneId(timezoneName, timezonesList, context) {
        if (!timezonesList || timezonesList.length === 0)
            return timezoneName;
        const tzLabel = (tz) => tz.zoneName || tz.name || '';
     ...
     */
    static parseOpenAIResponse(content) {
        try {
     ...
     */
    static getCurrencyFromTimezone(timezoneName) {
     ...
     */
    static getContextualTimezone(context, timezonesList, countriesData) {
        if (!timezonesList || timezonesList.length === 0) {
            ...
            for (const country of countriesData) {
                ...
                for (const term of countryTerms) {
                    if (contextLower.includes(term)) {
                        ...
                        const matchingTimezone = timezonesList.find(tz => tz.countryName?.toLowerCase().includes(country.name?.common?.toLowerCase()) ||
