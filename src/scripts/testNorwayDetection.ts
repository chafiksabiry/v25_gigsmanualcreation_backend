...
async function testNorwayDetection() {
  try {
    console.log('🧪 Test de détection de Norway...\n');
...
🚀 Lancement immédiat – équipe en cours de sélection. Ne rate pas ta place !, je veux le time zone et destination zone de Norway`;
    ...
    // Test avec des données de pays mock pour la conversion en ID
    const mockCountriesData = [
      {
        _id: '68c381ebf614d167d2c05999',
        name: {
          common: 'Norway',
    ...
    if (detectedCountry) {
      ...
      if (countryId === '68c381ebf614d167d2c05999') {
        console.log('✅ SUCCESS: Norway correctement détecté et converti en ID !');
      } else if (countryId === '68c381ebf614d167d2c05785') {
        console.log('❌ ERREUR: France détecté au lieu de Norway');
      } else {
        console.log('❌ ERREUR: ID inattendu ou pays non trouvé');
      }
    } else {
